angular
	.module("universe.admin.littleBirds")
	.service("littleBirdsService", littleBirdsService);

littleBirdsService.$inject = ["$q", "$http"];

function littleBirdsService($q, $http) {
	var littleBirdsService = {
		getAll: getAll,
		get: get,
		add: add,
		update: update,
	...
	return littleBirdsService;

	async function getAll() {
		// Simple GET request example:
		//TODO: store the url in a constant
		const response = await $http({
			method: "GET",
			url: "https://1-dot-aryaz-1.appspot.com/urldata/getAll"
		});
